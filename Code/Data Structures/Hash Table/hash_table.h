...
private:
    ...
    T1 *keys;
    T2 *values;
    int *status;  // 0 - пусто, 1 - занято, 2 - удалено
...
public:
    ...
    hash_table(int (*obj_to_int)(const T1 &));
...
template<typename T1, typename T2>
hash_table<T1, T2>::hash_table(int (*obj_to_int)(const T1 &)) {
...
template<typename T1, typename T2>
int hash_table<T1, T2>::primary_hash(int value) {
...
template<typename T1, typename T2>
int hash_table<T1, T2>::secondary_hash(int value) {
...
template<typename T1, typename T2>
int hash_table<T1, T2>::double_hash(const T1 &obj, int try_number) {
...
template<typename T1, typename T2>
int hash_table<T1, T2>::find_free_index(const T1 &key) {
...
template<typename T1, typename T2>
int hash_table<T1, T2>::get_index(const T1 &key) {
...
template<typename T1, typename T2>
void hash_table<T1, T2>::set(const T1 &key, const T2 &value, bool check_filling) {
...
template<typename T1, typename T2>
int hash_table<T1, T2>::get_index_status(int i) {
    return this->status[i];
}

template<typename T1, typename T2>
void hash_table<T1, T2>::set_new_size(int new_size, int new_power) {
...
template<typename T1, typename T2>
bool hash_table<T1, T2>::has(const T1 &key) {
    return get_index(key) != -1;
}

template<typename T1, typename T2>
T2 hash_table<T1, T2>::get(const T1 &key) {
    int index = this->get_index(key);
...
template<typename T1, typename T2>
void hash_table<T1, T2>::check_filling() {
...
template<typename T1, typename T2>
void hash_table<T1, T2>::remove(const T1 &key) {
    int index = this->get_index(key);
    if (index == -1) {  // Если ключа нет
        throw std::runtime_error("Error: Key does not exist");
    }
...
template<typename T1, typename T2>
void hash_table<T1, T2>::print_both_hashes(const T1 &key) {
...
template<typename T1, typename T2>
int hash_table<T1, T2>::get_count() {
    return this->curr_count;
}

template<typename T1, typename T2>
void hash_table<T1, T2>::remove(const T1 &key, const T2 &value) {
    int index = this->get_index(key);
    if (index == -1) {  // Если ключа нет
        throw std::runtime_error("Error: Key does not exist");
    }
 ...
 */
template<typename T1, typename T2>
std::vector<std::pair<T1, T2>> hash_table<T1, T2>::to_vector_status_1() {
...
template<typename T1, typename T2>
std::vector<std::pair<T1, T2>> hash_table<T1, T2>::to_vector_status_all() {
    std::vector<std::pair<T1, T2>> result = {};
    for (int i = 0; i < this->size; i++) {
        std::pair<T1, T2> p;
        p.first = this->keys[i];
